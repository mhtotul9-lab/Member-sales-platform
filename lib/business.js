...
const SETTINGS_DEFAULTS = {
  ...
  // A flat ৳ amount (NOT a percentage) added to the shared pool on every
  // valid sale, then split equally among every eligible-active member. This
  // is a single global figure the owner sets directly — entirely separate
  // from each product's own fixed per-unit member commission (see
  // processProductCommission below), which always goes 100% to the member
  // who made that specific sale.
  profitPoolShareAmount: 20,
  ...
  // Guards against configuring a product (commission + referral + pool
  // share) so generously that the company ends up keeping less than this
  // much — or even going negative — on a sale. Enforced when the admin
  // saves a product (see validateProductProfitSafety below).
  enableNegativeProfitProtection: true,
  minimumCompanyProfit: 0,
};
...
// Guards the owner from configuring a product so generously (commission +
// referral bonus + the flat pool contribution) that the company's own
// retained profit on that product would fall below the configured minimum
// (or go negative). Returns null when safe, or a Bengali error string
// naming the shortfall when the protection is enabled and would be violated.
export async function validateProductProfitSafety({ sellingPrice, costPrice, memberCommission, referralCommissionAmount }, settings) {
  const s = settings || (await getSettings());
  if (!s.enableNegativeProfitProtection) return null;

  const grossProfit = Number(sellingPrice) - Number(costPrice);
  const totalPayout = Number(memberCommission || 0) + Number(referralCommissionAmount || 0) + Number(s.profitPoolShareAmount || 0);
  const companyRemaining = grossProfit - totalPayout;
  const minRequired = Number(s.minimumCompanyProfit || 0);

  if (companyRemaining < minRequired) {
    return (
      `এই দাম/কমিশনে কোম্পানির লাভ নিরাপদ থাকছে না। প্রতি ইউনিটে গ্রস প্রফিট ৳${grossProfit.toFixed(2)}, ` +
      `কিন্তু মেম্বার কমিশন (৳${Number(memberCommission || 0).toFixed(2)}) + রেফারেল কমিশন (৳${Number(referralCommissionAmount || 0).toFixed(2)}) + ` +
      `প্রফিট পুল শেয়ার (৳${Number(s.profitPoolShareAmount || 0).toFixed(2)}) বিয়োগ করলে কোম্পানি রাখবে মাত্র ৳${companyRemaining.toFixed(2)}, ` +
      `যেখানে আপনার সেট করা সর্বনিম্ন প্রফিট ৳${minRequired.toFixed(2)}। Selling Price বাড়ান, Cost Price কমান, অথবা কমিশন/রেফারেল/পুল-শেয়ার কমান — ` +
      `অথবা Settings-এ গিয়ে "Negative Profit Protection" বন্ধ করলে জোর করে সেভ করা যাবে (সুপারিশ করা হয় না)।`
    );
  }
  return null;
}
...
export async function createProfitPool(orderRefId, order) {
  ...
  const eligibleMemberIds = await getEligibleActiveMemberIds();
  const poolRef = adminDb.collection("profitPools").doc();
  const now = new Date().toISOString();
  // The flat ৳ amount for this specific order was snapshotted at submission
  // time (order.poolShareAtOrder) — never re-read live from settings here,
  // so a later change to the global setting can't retroactively rewrite
  // history for orders that already happened.
  const poolAmount = Number(order.poolShareAtOrder) || 0;
