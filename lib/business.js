...
// Rolling-window check: does this member have at least one currently-valid
// (approved/processing/delivered/completed, i.e. never returned/refunded/
// rejected/cancelled afterward) sale approved within the last N days?
...
export async function getEligibleActiveMemberIds(settings) {
  ...
  const candidateIds = [...new Set(snap.docs.map((d) => d.data().memberId))];
...
// Called right after an order first enters a valid-sale status. Splits the
